...
def init(start, end, idx):
    ...
    mid = (start+end) // 2
    tree[idx] = init(start, mid, idx*2) * init(mid+1, end, idx*2+1) % (10**9 + 7)
    return tree[idx]
...
def find(start, end, idx, left, right):
    ...
    mid = (start + end) // 2
    return find(start, mid, idx*2, left, right) * find(mid+1, end, idx*2+1, left, right) % (10**9 + 7)
